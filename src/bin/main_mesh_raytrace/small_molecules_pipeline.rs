...
impl SmallMoleculesPipeline {
    pub fn new(device: &Device, depth_only: bool) -> Self {
        ...
        let fs = if depth_only { read_spirv(std::io::Cursor::new(&fs_depth_only[..])) } else { read_spirv(std::io::Cursor::new(&fs_color[..])) };
    ...
    pub fn create_bind_group(&self, device: &Device, camera: &Buffer, molecule: &Buffer, model_matrices: &Buffer, /*aabbs_rotations: &Buffer*/) -> BindGroup {
